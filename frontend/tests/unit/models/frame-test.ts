...
module("Unit | Model | frame", function(hooks) {
  setupTest(hooks);
  ...
  test("its computed properties are okay", function(assert) {
    ...
    const child: Frame = run(() => store.createRecord("frame", { name: "Child", parent: parent }));
  ...
  test("its functions work", function(assert) {
    ...
    assert.equal(parent.getSlot(slotName), undefined, "getSlot() returns undefined if there's not slot with given name");
  });
});
